 ...
 */
export function createParagraphBlock(content: string, htmlContent: string = ''): EditorBlock {
  return {
    id: uuidv4(),
 ...
 */
export function createTableBlock(rows: string[][], headerRow: boolean = true): EditorBlock {
  return {
    id: uuidv4(),
 ...
 */
export function createListBlock(items: string[], ordered: boolean = false): EditorBlock {
  return {
    id: uuidv4(),
...
export function createSectionWithBlocks(
  ...
  const sectionBlocks: EditorBlock[][] = Array(columns).fill(null).map(() => []);
