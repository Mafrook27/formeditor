...
function inheritedStylesToMarks(styles?: InheritedStyles): TextMark[] {
  ...
  if (
    styles.bold ||
    (styles.fontWeight && parseInt(styles.fontWeight) >= 600)
  ) {
  ...
  if (styles.fontFamily)
    marks.push({ type: "fontFamily", value: styles.fontFamily });
 ...
 */
function processTextWithPlaceholders(
  text: string,
  marks: TextMark[],
  segments: TextSegment[],
): void {
