...
export const ColorPicker = ({
  ...
  // Debounced onChange - only notify parent after user stops dragging
  useEffect(() => {
    if (onChange) {
...
export const ColorPickerSelection = memo(
  ({ className, ...props }: ColorPickerSelectionProps) => {
    ...
    const backgroundGradient = useMemo(() => {
      return `linear-gradient(0deg, rgba(0,0,0,1), rgba(0,0,0,0)),linear-gradient(90deg, rgba(255,255,255,1), rgba(255,255,255,0)),hsl(${hue}, 100%, 50%)`;
    }, [hue]);
    ...
    const handlePointerMove = useCallback(
      (event: PointerEvent) => {
        ...
        // Update color values (will be debounced by parent)
        setSaturation(x * 100);
...
export const ColorPickerFormat = ({
  ...
  const color = useMemo(() => {
    return Color.hsl(hue, saturation, lightness, alpha / 100);
